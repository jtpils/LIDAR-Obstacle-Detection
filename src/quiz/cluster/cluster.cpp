...
template <class T>
void print_vec(std::vector<T> pro) {
	for(auto p: pro) {
		std::cout << p << " ";
	}
	std::cout << std::endl;
...
void Proximity(bool ind, std::vector<bool> &processed, std::vector<int> &cluster, 
					const std::vector<std::vector<float>> points, KdTree *tree, float distTol) {
	processed[ind] = true;
	std::cout << "Boolean processed : " << std::endl;
	print_vec(processed);
	cluster.push_back(ind);
	std::vector<int> nearest = tree->search(points[ind], distTol);
	std::cout << "Nearest Points : " << std::endl;
	print_vec(nearest);
	for (int id : nearest) {
		if (!processed[id]) {
			Proximity(id, processed, cluster, points, tree, distTol);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int index=0; index<points.size(); index++) {
		...
		if (!processed[index]) {
			...
			Proximity(index, processed, clust, points, tree, distanceTol);
